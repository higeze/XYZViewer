...
void CFilterCell::PaintContent(CDC* pDC, CRect rcPaint)
{
	...
	if(!str.empty()){
		...
		pDC->DrawTextEx(const_cast<LPTSTR>(str.c_str()),-1,rcPaint,m_spProperty->GetFormat(),NULL);	
	}else{
		str = L"Filter items...";
		pDC->SetTextColor(RGB(210,210,210));
		pDC->DrawTextEx(const_cast<LPTSTR>(str.c_str()),-1,rcPaint,m_spProperty->GetFormat()&~DT_WORDBREAK|DT_END_ELLIPSIS,NULL);	
	}
