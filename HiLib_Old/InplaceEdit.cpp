...
//	if(!m_setter._Empty()){m_setter(GetWindowText());}
	if(!m_changed._Empty()){m_changed(GetWindowText());}
	...
	dc.DrawTextEx(const_cast<LPTSTR>(strText.c_str()),-1,&rcCalc,m_spDrawTextProperty->GetFormat() | DT_CALCRECT,NULL);
