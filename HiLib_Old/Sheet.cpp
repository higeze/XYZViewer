...
void CSheet::Sort(CColumn* pCol, Sorts sort)
{
	...
	case Sorts::Down:
		std::stable_sort(vRowPlusData.begin(),vRowPlusData.end(),[pCol](const RowData& lhs,const RowData& rhs)->bool{
			return _tcsicmp(pCol->Cell(lhs.DataPtr.get())->GetSortString().c_str(),pCol->Cell(rhs.DataPtr.get())->GetSortString().c_str())>0;
		});
		break;
	case Sorts::Up:
		std::stable_sort(vRowPlusData.begin(),vRowPlusData.end(),[pCol](const RowData& lhs,const RowData& rhs)->bool{
...
CSheet::coordinates_type CSheet::GetColumnInitWidth(CColumn* pColumn)
{
	...
	auto dc = CDC(::CreateCompatibleDC(*GetClientDCPtr().get()));	
...
CRect CSheet::GetRect()const
{
	...
	rc.InflateRect((coordinates_type)ceil(penWidth*0.5), (coordinates_type)ceil(penWidth*0.5));
	return rc;
}
