...
{
	coordinates_type penWidth(m_spProperty->GetPenPtr()->GetWidth());
	rcCenter.DeflateRect((coordinates_type)floor(penWidth*0.5),(coordinates_type)floor(penWidth*0.5));
	return rcCenter;
}
...
CRect CCell::InnerBorder2CenterBorder(CRect rcInner)
{
	...
	coordinates_type penWidth=m_spProperty->GetPenPtr()->GetWidth();
	rcInner.InflateRect((coordinates_type)floor(penWidth*0.5),(coordinates_type)floor(penWidth*0.5));
	return rcInner;
}
...
void CCell::PaintBackground(CDC* pDC, CRect rcPaint)
{
	...
	if(GetSelected()){
		...
		rgb.SetR(rgbSelected.GetR()*alpha + rgb.GetR()*(1-alpha)); 
		rgb.SetG(rgbSelected.GetG()*alpha + rgb.GetG()*(1-alpha)); 
		rgb.SetB(rgbSelected.GetB()*alpha + rgb.GetB()*(1-alpha)); 
		cr.SetRGB(rgb);
		br = CBrush(cr);
	}
