...
public:
	COGLLineProperty() {}
	COGLLineProperty(float width, CColor color)
...
public:
	COGLPointProperty() {}
	COGLPointProperty(float size, CColor color)
...
private:
	...
	std::shared_ptr<COGLLineProperty> m_spLine;
	std::shared_ptr<COGLPointProperty> m_spPoint;
public:
	COGLSeriesProperty() {};
	COGLSeriesProperty(int fontSize,std::wstring fontName, int lineWidth, CColor lineColor, int pointSize, CColor pointColor)
		:m_spFont(std::make_shared<CFont>(fontSize, fontName)),
		m_spLine(std::make_shared<COGLLineProperty>( lineWidth, lineColor)),
		m_spPoint(std::make_shared<COGLPointProperty>(pointSize, pointColor)){}
	virtual ~COGLSeriesProperty(){}
	std::shared_ptr<CFont> GetFontPtr()const{return m_spFont;}
	std::shared_ptr<COGLLineProperty> GetLinePtr()const { return m_spLine; }
	void SetLinePtr(std::shared_ptr<COGLLineProperty> spLine) { m_spLine = spLine; }
	std::shared_ptr<COGLPointProperty> GetPointPtr()const{return m_spPoint;}
...
public:
	...
	{
		CColor black = RGB(0, 0, 0);
		std::array<GLubyte, 3> blackArray = { 0,0,0 };
		CColor lightGray = RGB(220, 220, 220);

		m_spAxisProperty = std::make_shared<COGLAxisProperty>(std::make_shared<COGLLineProperty>(2.0f, black), std::make_shared<COGLFontProperty>(0.7f, blackArray));
		m_spTickProperty = std::make_shared<COGLTickProperty>(std::make_shared<COGLLineProperty>(2.0f, black), std::make_shared<COGLFontProperty>(0.5f, blackArray));
...
public:
	...
	virtual void SetState(const UIElementState::Type& state)
	{
		if(GetState()!=UIElementState::Hot && state == UIElementState::Hot){
			auto spLine = m_getter()->GetLinePtr();
			m_getter()->SetLinePtr(std::make_shared<COGLLineProperty>(spLine->GetWidth()*2, spLine->GetColor()));
