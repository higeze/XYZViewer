...
void COGLChartLegend::OnContainerChanged(NotifyContainerChangedEventArgs<std::shared_ptr<COGLSeries>>& e)
{
	...
	case NotifyContainerChangedAction::Add:
		boost::for_each(e.NewItems,[&](std::shared_ptr<COGLSeries> spSeries){
			...
			auto spTextBlock = std::make_shared<CBindTextBlock>(spStackPanel.get(), 
			...
			spTextBlock->LButtonDblClk.connect([spSeries](MouseEventArgs& e)->void {
				CRect rc(0, 0, 0, 0);
				CGridViewProperty gridProp;
				auto pPropWnd = new CPropertyWnd<COGLSeriesProperty>(
					gridProp.m_spBackgroundProperty,
					gridProp.m_spPropHeader,
					gridProp.m_spPropCell,
					gridProp.m_spPropCell,
					gridProp.m_spPropHeader,
					gridProp.m_spPropCell,
					gridProp.m_spPropCell,
					gridProp.m_spDeltaScroll,
					L"Series Property",
					spSeries->GetSeriesPropertyPtr());

				//pPropWnd->PropertyChanged.connect([&](const std::wstring& str)->void {
				//	for (auto iter = m_gridViewMap.begin(); iter != m_gridViewMap.end(); ++iter) {
				...
				//});
				pPropWnd->Create(e.WindowPtr->m_hWnd, rc);
			...
			spChildren->push_back(spTextBlock);
