...
{
	if(!m_bVisible)return;
	CPen pen(PS_SOLID, m_getter()->GetLinePtr()->GetWidth(), m_getter()->GetLinePtr()->GetColor());
	HPEN hPen = e.DCPtr->SelectPen(pen);

	//CBrush brush()
	//HBRUSH hBrush = e.DCPtr->SelectBrush(brush);
...
void C3DLineChart::OnLButtonDblClk(MouseEventArgs& e)
{
	for (auto& child : *m_children) {
		if (child->GetRect().PtInRect(e.Point)) {
			child->OnLButtonDblClk(e);
		}
	}
}
